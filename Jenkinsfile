...
pipeline {
    ...
    stages {
        ...
        stage('Checkout To Mcroservice Branch'){
            steps{
                git branch: 'app-recommendation-service', url: 'https://github.com/awanmbandi/realworld-microservice-project.git'
            }
        }
        ...
        stage("SonarQube SAST Analysis"){
            steps{
                withSonarQubeEnv('Sonar-Server') {
                    sh ''' $SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=app-recommendation-service \
                    -Dsonar.projectKey=app-recommendation-service '''
                }
            }
        }
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker build -t awanmbandi/recommendationservice:latest ."
                    }
                }
            }
        }
        ...
        //         script {
        //             withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
        //                 sh "docker push awanmbandi/recommendationservice:latest "
        //             }
        //         }
        //     }
        // }
