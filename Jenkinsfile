...
pipeline {
    ...
    stages {
        ...
        stage('Checkout To Mcroservice Branch'){
            steps{
                git branch: 'app-loadgenerator-service', url: 'https://github.com/awanmbandi/realworld-microservice-project.git'
            }
        }
        ...
        stage("SonarQube SAST Analysis"){
            steps{
                withSonarQubeEnv('Sonar-Server') {
                    sh ''' $SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=app-loadgenerator-service \
                    -Dsonar.projectKey=app-loadgenerator-service '''
                }
            }
        }
